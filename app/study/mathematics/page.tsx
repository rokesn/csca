// Mathematics study page — same full-syllabus layout as /syllabus/mathematics:
// hero + stats + sticky module nav + modules → chapters → topic-name rows,
// each topic opening its 5-stage lesson.
    ...
    <main className="text-slate-200">
      ...
      <section className="study-grid-bg border-b border-white/10">
        <div className="mx-auto max-w-4xl px-4 pb-10 pt-10">
          <BackLink href="/study">← Study</BackLink>
          <div className="mt-4 flex items-start gap-4 sm:gap-5">
            ...
            <div>
              <div className="flex flex-wrap items-center gap-2">
                <Eyebrow tone={tone}>Study · Mathematics</Eyebrow>
              ...
              <p className="mt-2 max-w-2xl text-sm leading-relaxed text-slate-400 sm:text-base">
                {MATH_SYLLABUS.length} chapters · {MATH_TOPIC_COUNT} topics.
                Follow the roadmap in order — open a topic to start its 5
                stages: Learn → See → Practice → Timed → Mastery.
              </p>
            </div>
          </div>
                  ...
                  <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1">
                    {ch.id === "sets-inequalities" ? (
                      <Link
                        ...
                        className="group font-display text-lg font-bold text-white hover:text-indigo-200"
                      >
                        {ch.title}
                        <span aria-hidden="true" className="ml-1.5 inline-block transition group-hover:translate-x-0.5">→</span>
                      </Link>
                    ) : (
                    ...
                    <div className="mt-3">
                      ...
                      >
                        <span className="flex-1 text-sm font-bold text-slate-100 transition group-hover:text-white">
                          Open full chapter: 16 lessons + {SETS_ALL.length} problems
                        </span>
                        <span
                          aria-hidden="true"
                    ...
                    {ch.topics.map((t, ti) => (
                      <li key={t.id}>
                        <Link
                          href={
                            ch.id === "sets-inequalities"
                              ? "/study/mathematics/sets-inequalities"
                              : `/study/mathematics/${t.slug}`
                          }
                          className="group flex items-center gap-3 rounded-xl border border-white/10 bg-white/[0.03] px-4 py-3 transition duration-200 hover:-translate-y-0.5 hover:border-white/25 hover:bg-white/[0.05]"
                        >
                          <span
